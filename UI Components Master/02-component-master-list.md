...
| C03 | Plan Selector | L3 | P0 | `tier-cards` (Modular), `configurator` (Flex) | `default` (no selection), `selected` (highlighted tier/SI), `disabled` (grade-restricted) | Modular: tier-cards (Tier Upgrade), Flex: configurator, Vanilla: skip | `PlanSelector/TierCards`, `PlanSelector/Configurator` |
...
| C08 | Top-up Card | L4 | P1 | `standard`, `tier-upgrade` (Modular), `flex` | `available` (can toggle), `selected` (toggled on) | Only when `topUp` is configured | `Card/TopUp` |
...
| C10 | Wallet Display | L1, L3, L4, L5 | P0-FLEX | `banner` (full bar + labels), `detailed` (with breakdown), `inline` (compact bar + remaining text), `mini` (thin bar only) | `full` (under budget), `partial` (partially used), `exceeded` (over wallet, cerise bar + warning) | Flex only (both Base Variable and Base Fixed) | `Wallet/Banner`, `Wallet/Detailed`, `Wallet/Inline`, `Wallet/Mini` |
...
| C03 Plan Selector | N/A (skip L3) | tier-cards (Tier Upgrade: 3 tiers) | configurator (Base Variable: SI + family) / view-only (Base Fixed) |
...
| C08 Top-up Card | standard | tier-upgrade | standard + from wallet |
| C09 Secondary Card | single | single | single + from wallet |
| C10 Wallet | N/A | N/A | Base Variable: banner/detailed/inline/mini &#124; Base Fixed: banner/inline |
